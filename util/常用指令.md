...
if [ "$color_prompt" = yes ]; then
    PS1='${debian_chroot:+($debian_chroot)}\[\033[01;32m\]\u@\h\[\033[00m\]\[\033[01;37m\][\t]\[\033[00m\]:\[\033[01;34m\]\w\[\033[00m\]\$ '
else
...
# SSH
## SSH连接到指定端口
使用-p选项：`ssh -p xx user@ip` ，xx 为端口号，user为用户名，ip为要连接的ip
...
```
表示：本地SSH Client每隔60s向Server端SSHD发送 keep-alive 包，如果发送30次, Server端还无回应则断开连接。

---

## SSH端口转发
用-R选项：`ssh -R 16666:127.0.0.1:8118 zhaojp@10.60.57.78 -p 2887
`
...
## 获取当前文件夹名
```bash
curdirname=`echo $PWD | awk -F"/" '{print $NF}'`
echo curdirname 
```